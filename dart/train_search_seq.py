...
from model_search_seq import Network
...
def create_input_sequence(data, target, num_days=5):
    input_sequences = []
    target_values = []

    for i in range(num_days, len(target)):
        if not np.isnan(target[i]):
            input_sequence = data[i - num_days + 1:i + 1]  # Previous 5 days
...
def read_data():
    ...
    input_sequences, target_values = create_input_sequence(reshaped_data, reshaped_target, num_days=5)

    # Convert NumPy arrays to PyTorch tensors
    data_tensor = torch.Tensor(input_sequences)
    ...
    train_loader = DataLoader(
        dataset, batch_size=6,  # Assuming batch size is 6 as per your requirement
